...
def rerank_by_overlap(contexts: list[str], query: str) -> list[str]:
    ...
    return sorted(
        contexts, key=lambda c: len(_tokenize(c) & query_tokens), reverse=True
    )
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric:\n{criteria_lines}\n"
            "Score each criterion from 0 to 1 and return JSON."
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        multi_criterion_entries = 0
        first_is_highest_count = 0

        for entry in scores_batch:
            values = list(entry.get("scores", {}).values())
            all_scores.extend(values)
            if len(values) > 1:
                multi_criterion_entries += 1
                if values[0] == max(values):
                    first_is_highest_count += 1

        positional_bias = (
            multi_criterion_entries > 0
            and first_is_highest_count == multi_criterion_entries
        )
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        lowest_metrics = [name for name, value in scores.items() if value == lowest_value]
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append("Implement a hallucination checker to filter claims unsupported by context")
        if categories.get("irrelevant"):
            suggestions.append("Improve prompt clarity or intent detection so answers stay on-topic")
        ...
        if categories.get("off_topic"):
            suggestions.append("Review intent detection — answers are addressing the wrong topic")

        fallback_suggestions = [
            "Add few-shot examples showing complete answers to improve completeness",
            "Expand the golden dataset with more edge cases similar to these failures",
            "Review retrieval quality — check chunking and embedding relevance",
        ]
